...
A mock never pauses, so an attempt whose student closed the tab mid-module stays ACTIVE
with an expired clock forever: it never reaches SCORING, never scores, and — via
``uniq_active_solo_mock_attempt`` — permanently blocks that student from starting the mock
again. Reaping fast-forwards it to completion; unanswered questions grade as omitted, which
is the right outcome for a timed test nobody came back to.
"""
